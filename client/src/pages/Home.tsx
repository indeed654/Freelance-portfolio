  ...
  return (
    <div className="project-modal-backdrop" onClick={onClose} role="presentation">
      <div className="project-modal" role="dialog" aria-modal="true" aria-labelledby="project-modal-title" onClick={(event) => event.stopPropagation()}>
        ...
        {project.gallery && project.gallery.length > 1 ? <div className="modal-gallery">{project.gallery.map((image, index) => <img key={image} src={image} alt={`${project.name} project screen ${index + 1}`} loading="lazy" />)}</div> : null}
        ...
        <section className="pricing-section section-ink">
          <div className="container">
            ...
            <div className="pricing-grid">
              {pricing.map((packageItem) => <article className={`price-card ${packageItem.featured ? "price-card-featured" : ""}`} key={packageItem.name}>{packageItem.featured ? <span className="price-featured-tag">MOST REQUESTED</span> : null}<span className="price-eyebrow">{packageItem.eyebrow}</span><h3>{packageItem.name}</h3><p>{packageItem.description}</p><p className="price-ideal"><strong>Best for</strong>{packageItem.idealFor}</p><div className="price-timeline"><span>Typical timeline</span><strong>{packageItem.timeline}</strong></div><strong className="price-value">{packageItem.price}</strong><div className="price-divider" />{packageItem.features.map((feature) => <div className="price-feature" key={feature}><Check size={15} />{feature}</div>)}<button className="price-link" onClick={() => scrollToSection("contact")}>Discuss this package <ArrowUpRight size={16} /></button></article>)}
            </div>
            <div className="pricing-breakdown"><div className="breakdown-heading"><span>WHAT EACH PART COVERS</span><p>Every project is scoped around the real brief. This is the working breakdown behind the package starting points.</p></div><div className="breakdown-table"><div className="breakdown-row breakdown-row-head"><span>Project stage</span><span>Starter</span><span>Professional</span><span>Premium</span></div>{pricingBreakdown.map((row) => <div className="breakdown-row" key={row.stage}><strong>{row.stage}</strong><span data-label="Starter">{row.starter}</span><span data-label="Professional">{row.professional}</span><span data-label="Premium">{row.premium}</span></div>)}</div></div>
